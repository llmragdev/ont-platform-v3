...
def old_similarity_method(sentence1, sentence2):
    # 예전 방식: 비교할 때마다 매번 새로운 단어 사전을 만듦
    sentences = (sentence1, sentence2)
    tfidf_vectorizer = TfidfVectorizer()
    
    try:
        tfidf_matrix = tfidf_vectorizer.fit_transform(sentences)
        # 첫 번째(질문)와 두 번째(청크) 문장 비교
...
print(f"질문: '{query}' (예전 TF-IDF 방식)\n")
print("-" * 50)

for i, chunk in enumerate(chunks):
    sim = old_similarity_method(query, chunk)
