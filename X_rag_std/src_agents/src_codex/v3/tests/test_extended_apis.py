...
def test_delete_document_removes_search_result(client):
    ...
    delete = client.delete(f"/api/v1/documents/{doc_id}", headers={"X-Tenant-ID": "company_a"})
...
def test_chroma_adapter_stores_provider_embeddings(monkeypatch):
        ...
        [
            {
                ...
                "category_mid": "policy",
                "vector_db_id": "vdb_policy_01",
                "tags": ["must_not_be_in_chroma_metadata"],
            }
        ],
    )
    ...
    assert "tags" not in calls["add"]["metadatas"][0]
    assert calls["add"]["metadatas"][0]["org_id"] == ""
