...
def test_gemini_embedding_uses_gateway_without_api_key(monkeypatch):
    ...
    assert service.embed_text("hello", tenant_id="company_a") == [0.1, 0.2, 0.3]
    assert requests == [
        {
            "url": "http://gateway:8010/api/v1/embed",
            "json": {"text": "hello", "tenant_id": "company_a"},
            "timeout": 30.0,
        }
    ]
...
def test_gemini_llm_uses_gateway_without_api_key(monkeypatch):
    ...
    assert client.generate_answer("question", [chunk], tenant_id="company_a") == "ok"
    assert requests[0]["url"] == "http://gateway:8010/api/v1/generate"
    assert requests[0]["json"]["tenant_id"] == "company_a"
