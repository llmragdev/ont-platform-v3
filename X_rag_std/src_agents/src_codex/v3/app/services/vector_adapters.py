...
class LocalJsonVectorDbAdapter(BaseVectorDbAdapter):
    ...
    def _matches_filters(metadata: dict, filters: dict) -> bool:
        org_policy = filters.get("__org_or_null__")
        for key, expected in filters.items():
            if key == "__org_or_null__":
                continue
            if expected is None:
                continue
        ...
        if org_policy:
            if metadata.get("org_id") is None:
                return True
            if "dept_code" in org_policy:
                return metadata.get("dept_code") == org_policy["dept_code"]
            if "org_id" in org_policy:
                return metadata.get("org_id") == org_policy["org_id"]
        return True
...
class ChromaVectorDbAdapter(BaseVectorDbAdapter):
    ...
    def add_documents(self, chunks: list[dict], metadata: list[dict]) -> bool:
        ...
        vector_metadata = [
            {
                key: ("" if key == "org_id" and value is None else value)
                for key, value in meta.items()
                if key != "tags" and (value is not None or key == "org_id")
            }
    ...
    def _build_where(filters: dict) -> dict | None:
        ...
        for key, value in filters.items():
            if key == "__org_or_null__" or value is None:
                continue
            conditions.append({key: {"$eq": value}})
        if org_policy:
            if "dept_code" in org_policy:
                conditions.append(
                    {"$or": [{"dept_code": {"$eq": org_policy["dept_code"]}}, {"org_id": {"$eq": ""}}]}
                )
            elif "org_id" in org_policy:
                conditions.append(
                    {"$or": [{"org_id": {"$eq": org_policy["org_id"]}}, {"org_id": {"$eq": ""}}]}
                )
        if not conditions:
            return None
        if len(conditions) == 1:
