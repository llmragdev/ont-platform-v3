...
class RagSearchService:
    ...
    def search(
        ...
        query_vector = self._embed_query(request.query, context.tenant_id)
        raw_candidates = adapter.search(
            query_vector=query_vector,
            top_k=max(request.top_k, 1),
            filters=self._adapter_filters(filters, context),
        )
        ...
        answer = self.llm_client.generate_answer(request.query, used_chunks, tenant_id=context.tenant_id)
        self.dialog_repository.save(
            request.query,
            answer,
            used_chunks,
    ...
    @staticmethod
    def _adapter_filters(filters: RagSearchFilter, context: RequestContext) -> dict:
        values = filters.model_dump(exclude_none=True)
        values.pop("vector_db_id", None)
        values["tenant_id"] = context.tenant_id
        if context.scope_level == "department" and context.dept_code:
            values["__org_or_null__"] = {"dept_code": context.dept_code}
        elif context.scope_level == "team" and context.org_id:
            values["__org_or_null__"] = {"org_id": context.org_id}
        return values

    def _embed_query(self, query: str, tenant_id: str) -> list[float]:
        try:
            return self.embedding_service.embed_text(query, tenant_id=tenant_id)
        except TypeError:
