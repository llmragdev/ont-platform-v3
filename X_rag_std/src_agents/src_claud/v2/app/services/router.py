...
class VectorDbRouter:
    ...
    def get_adapter(
        ...
        # VECTOR_DB_ENGINE 환경변수가 최우선, 없으면 라우팅 규칙의 engine_type 사용
        engine = settings.vector_db_engine
        if engine == "local_json" and rule:
            engine = rule.engine_type
        ...
        if engine == "chroma":
            ...
            return ChromaAdapter(
                ...
                host=conn.get("host", settings.chroma_host),
                port=conn.get("port", settings.chroma_port),
