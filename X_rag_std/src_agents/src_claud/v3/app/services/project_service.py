...
class ProjectService:
    ...
    def create(
        ...
        self._audit_repo.log("create_project", tenant_id=tenant_id, resource=proj.project_code)
        return proj
    ...
    def delete(self, project_code: str, tenant_id: str = "default") -> None:
        proj = self._repo.get(project_code)
        if not proj:
            raise ProjectNotFoundError(f"project_code={project_code} not found")
        ...
        self._audit_repo.log("delete_project", tenant_id=tenant_id, resource=project_code)
