# 메달리온 아키텍처 기반 테이블 및 쿼리 관리 설계서 (⚠️ DEPRECATED)

> **⚠️ 이 문서는 더 이상 사용되지 않습니다.**
> 
> **대신 [`design/파이프라인빌더/11_데이터_추상화_계층_설계_및_메달리온_비교.md`](../../파이프라인빌더/11_데이터_추상화_계층_설계_및_메달리온_비교.md)를 참고하세요.**
> 
> **변경 사유:**
> - 메달리온(Bronze/Silver/Gold)은 저장소 비용 **3배 증가** (→ 메달리온: $475/월, 추상화: $250/월)
> - 온톨로지 플랫폼의 특성(의미론적 계층)과 충돌
> - **Snowflake View 기반 추상화 계층이 효율적** (저장소 50% 절감, 온톨로지 통합 용이)
> - 모든 환경(Dev/Stag/Prod)에서 **일관된 뷰 사용 가능** → 배포 복잡도 감소
> - 아카이브: [`design/파이프라인빌더/archive/`](../../파이프라인빌더/archive/)
...
**문서 번호**: `design/AI코딩에이전트/10_메달리온_아키텍처_기반_테이블_및_쿼리_관리_설계.md` **(DEPRECATED)**
...
3. **확장성**: Snowflake 커넥터를 교체하는 것만으로 현장 데이터 환경을 즉시 Cloud Data Warehouse 스케일로 연동할 수 있는 단단한 기초 메타데이터 아키텍처를 확보합니다.
