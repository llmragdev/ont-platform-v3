...
export default function Home() {
  ...
  async function executeAction(action: string) {
    setActionError(null);
    if (backendStatus === "online" && selectedOrder?.docId) {
      try {
        ...
        const stillSelected = refreshed.find((o) => o.id === selectedOrderId);
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                  {selectedOrder.availableActions.length > 0 ? (
                    selectedOrder.availableActions.map((action) => (
                      <div
                        key={action}
                        className="rounded-xl border border-slate-100 px-3 py-2.5 bg-white shadow-sm"
                      >
                        <span className="text-xs font-bold text-slate-700">
                          {action}
                        </span>
              ...
              {selectedOrder.docId && (
                <div className="space-y-3">
                  ...
                  <div className="rounded-2xl bg-slate-50 p-5 border border-slate-100">
                    <ContextItem label="Doc ID" value={selectedOrder.docId} />
                  </div>
                </div>
              )}
            </div>
