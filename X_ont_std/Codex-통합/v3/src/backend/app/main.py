...
def v1_execute_action(
    ...
    require_permission(tenant, "can_edit_object")
    request = body or ActionExecuteRequest(
        action_name=action_name or "",
        target_id=target_id or "",
        params={},
    )
    return ActionService(tenant).execute_action(
        request.action_name,
        request.target_id,
        request.params,
...
@app.post("/api/v1/ontology/materialize")
def v1_materialize(
    body: MaterializeRequest,
    tenant: TenantContext = Depends(current_context),
) -> dict:
    require_permission(tenant, "can_read")
    return MaterializeService(tenant).materialize(
...
@app.get("/api/v1/ontology/writeback")
def v1_writeback_requests(tenant: TenantContext = Depends(current_context)) -> dict:
    require_permission(tenant, "can_read")
    return {"writeback_requests": WriteBackService(tenant).list_requests()}
