...
class WorkflowService:
    ...
    def _build_engine(self) -> None:
        ...
        def policy_allows(order: ObjectInstance, payload: dict) -> bool:
            ...
            return self.policy.can_execute_action(payload["user"], payload["action"], context["order"], context["customer"])

        self.engine.register_transition(WorkflowTransition("SubmitToApproved", order_type, "Submitted", "ApproveOrder", "Approved", policy_allows))
        self.engine.register_transition(WorkflowTransition("ReviewToApproved", order_type, "Review", "ApproveOrder", "Approved", policy_allows))
        self.engine.register_transition(WorkflowTransition("SubmitToRejected", order_type, "Submitted", "RejectOrder", "Rejected", policy_allows))
        self.engine.register_transition(WorkflowTransition("ReviewToRejected", order_type, "Review", "RejectOrder", "Rejected", policy_allows))
        self.engine.register_transition(WorkflowTransition("SubmitToReview", order_type, "Submitted", "HoldOrder", "Review", policy_allows))
        self.engine.register_transition(WorkflowTransition("ApprovedToFulfilled", order_type, "Approved", "FulfillOrder", "Fulfilled"))
        self.engine.register_transition(WorkflowTransition("FulfilledToClosed", order_type, "Fulfilled", "CloseOrder", "Closed"))
    ...
    def queue(self, user: dict) -> list[dict]:
        ...
        for order in self.ontology.orders():
            try:
                ...
                if actions:
                    rows.append({**context["order"], "customer": context["customer"], "available_actions": actions})
            except AppError:
                continue
        return rows
