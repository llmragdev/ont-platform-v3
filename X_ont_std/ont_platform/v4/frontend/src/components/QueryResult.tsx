...
function displayCell(value: string | undefined): { text: string; title: string; empty: boolean } {
  const raw = value ?? "";
  if (!raw) return { text: "-", title: "", empty: true };
  if (raw.length > 500) return { text: `${raw.slice(0, 500)}...`, title: raw, empty: false };
  return { text: raw, title: raw, empty: false };
}
            ...
            {rows.length === 0 ? (
              ...
              <>
                {rows.length > visibleRows.length && (
                  <div data-testid="result-render-limit" className="border-b border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-500">
                    Showing first {visibleRows.length} of {rows.length} rows for faster rendering.
                  </div>
                )}
                <table className="data-table">
                  ...
                  <tbody>
                    {visibleRows.map((row, index) => (
                      <tr key={index}>
                        {columns.map((column) => {
                          const cell = displayCell(row[column]);
                          return (
                            <td
                              key={column}
                              data-empty-cell={cell.empty ? "true" : undefined}
                              data-value={cell.empty ? undefined : "true"}
