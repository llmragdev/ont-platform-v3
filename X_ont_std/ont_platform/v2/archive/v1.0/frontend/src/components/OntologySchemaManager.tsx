...
export function OntologySchemaManager() {
  ...
  const loadSchema = useCallback(async () => {
    setLoading(true);
    try {
      const s = await api.ontologyMgmt.getSchema();
      setSchema(s);
    } catch (err) {
  ...
  async function handleAddEntityType() {
    ...
    try {
      ...
      await api.ontologyMgmt.addEntityType({ name: etName.trim().toUpperCase(), description: etDesc.trim(), properties: props }, userId);
  ...
  async function handleDeleteEntityType(name: string) {
    try {
      await api.ontologyMgmt.deleteEntityType(name, userId);
  ...
  async function handleAddRelationType() {
    ...
    try {
      await api.ontologyMgmt.addRelationType({ name: rtName.trim().toUpperCase(), from_type: rtFrom.trim().toUpperCase(), to_type: rtTo.trim().toUpperCase() }, userId);
  ...
  async function handleDeleteRelationType(name: string) {
    try {
      await api.ontologyMgmt.deleteRelationType(name, userId);
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && (
        <div className="text-sm text-slate-500">스키마 로딩 중…</div>
      )}
      ...
      <section>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {schema?.entity_types.filter((t) => t.is_builtin).map((t) => (
              <div
                key={t.name}
                className={`rounded-lg border px-3 py-2 text-xs ${BUILTIN_COLORS[t.name] ?? "bg-slate-50 text-slate-700 border-slate-200"}`}
              >
              ...
              {schema?.entity_types.filter((t) => !t.is_builtin).map((t) => (
                <div key={t.name} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2">
                  <div>
                    ...
                    {t.properties.length > 0 && (
                      <span className="ml-2 text-xs text-slate-400 font-mono">[{t.properties.join(", ")}]</span>
                    )}
                  </div>
                  {canEditOntology && (
                    <button
                      className="text-rose-400 hover:text-rose-600 text-xs ml-2"
                      onClick={() => void handleDeleteEntityType(t.name)}
                    >
                      삭제
                    </button>
                  )}
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <label className="block text-xs text-slate-500 mb-1">유형명 (영대문자 권장)</label>
              <input
          ...
          <div className="mb-3">
            ...
            <input
              className="w-full border border-slate-200 rounded px-2 py-1.5 text-sm bg-white"
              placeholder="예: start_date, end_date, value"
              value={etProps}
              onChange={(e) => setEtProps(e.target.value)}
            />
          </div>
          <button
            ...
            {schema?.relation_types.map((r) => (
              <div key={r.name} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2">
                ...
                {canEditOntology && (
                  <button
                    className="text-rose-400 hover:text-rose-600 text-xs ml-2"
                    onClick={() => void handleDeleteRelationType(r.name)}
                  >
                    삭제
                  </button>
                )}
