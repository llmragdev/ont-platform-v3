    ...
    <div className="space-y-4">
      ...
      {result && (
        <>
          ...
          <section className="panel">
            <div className="panel-header"><h3 className="text-sm font-semibold">AI 답변</h3></div>
            <div className="panel-body space-y-2">
              ...
              {result.trace && result.trace.length > 0 && (
                <div className="text-xs text-slate-400 pt-2 border-t border-slate-100">
                  Trace: {result.trace.slice(0, 5).join(" → ")}
                </div>
              )}
            </div>
          </section>

          {(result.sources ?? []).filter((s) => s.source_type === "vector").length > 0 && (
            <section className="panel">
              <div className="panel-header">
                <h3 className="text-sm font-semibold">벡터 검색 결과</h3>
                <span className="text-xs text-slate-500">{result.structured_data?.vector?.count ?? 0}건</span>
              </div>
              <div className="panel-body space-y-2">
                {(result.sources ?? []).filter((s) => s.source_type === "vector").map((s, i) => (
                  <div key={i} className="rounded-md border border-slate-200 p-3">
                    <div className="text-xs font-medium text-slate-600">{s.citation}</div>
                  </div>
                ))}
              </div>
            </section>
