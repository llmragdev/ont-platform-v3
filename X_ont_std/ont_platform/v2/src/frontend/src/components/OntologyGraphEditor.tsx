...
function OntologyNode({ id, data, selected }: NodeProps) {
  ...
  return (
    <div style={{ background: c.bg, border: `2px solid ${selected ? "#2563eb" : c.border}`, borderRadius: 10, padding: "8px 14px", minWidth: 130, boxShadow: selected ? "0 0 0 3px #bfdbfe" : "0 1px 4px rgba(0,0,0,.08)" }}>
      <Handle type="target" position={Position.Top} style={{ background: c.border }} />
      <div style={{ color: c.text, fontSize: 9, fontWeight: 700, letterSpacing: 1, marginBottom: 2, textTransform: "uppercase" }}>{(data as { type?: string }).type ?? "?"}</div>
...
export function OntologyGraphEditor() {
  ...
  async function handleAddRelationship() {
    ...
    try {
      await api.ontologyMgmt.addRelationship(selectedDoc, newRel);
  ...
  return (
    <div className="flex gap-4" style={{ height: 680 }}>
      <div className="flex-1 rounded-xl border border-slate-200 overflow-hidden relative">
        {loading && (
          <div className="absolute inset-0 flex items-center justify-center bg-white/70 z-10 text-sm text-slate-500">로딩 중…</div>
        )}
        ...
        <div className="absolute bottom-12 left-2 flex flex-wrap gap-1 z-10 max-w-xs">
          {Object.entries(TYPE_COLORS).map(([t, c]) => (
            <span key={t} className="px-1.5 py-0.5 rounded text-[10px] font-medium" style={{ background: c.bg, color: c.text, border: `1px solid ${c.border}` }}>{t}</span>
          ))}
        </div>
      </div>
      ...
      <div className="w-72 shrink-0 rounded-xl border border-slate-200 bg-white overflow-y-auto p-4">
        {selectedNode ? (
          <>
            ...
            <div className="rounded-md px-3 py-2 mb-3 text-xs font-semibold" style={{ background: TYPE_COLORS[selectedNode.type]?.bg ?? "#f8fafc", color: TYPE_COLORS[selectedNode.type]?.text ?? "#475569", border: `1px solid ${TYPE_COLORS[selectedNode.type]?.border ?? "#94a3b8"}` }}>
              ...
              <div className="mb-3">
                ...
                {outEdges.map((e) => (
                  <div key={e.id} className="flex items-center justify-between py-1 border-b border-slate-50">
                    <span className="text-xs text-slate-600"><span className="font-medium text-blue-600">{e.label}</span> → {e.to}</span>
                    {canEditDiagram && <button className="text-rose-400 hover:text-rose-600 text-xs" onClick={() => void handleDeleteRelationship(e.id)}>✕</button>}
                  </div>
                ))}
              </div>
            )}
            {inEdges.length > 0 && (
              <div className="mb-3">
                ...
                {inEdges.map((e) => (
                  <div key={e.id} className="flex items-center justify-between py-1 border-b border-slate-50">
                    <span className="text-xs text-slate-600">{e.from} → <span className="font-medium text-amber-600">{e.label}</span></span>
                    {canEditDiagram && <button className="text-rose-400 hover:text-rose-600 text-xs" onClick={() => void handleDeleteRelationship(e.id)}>✕</button>}
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-400 text-xs text-center gap-2 py-12">
            ...
            <p className="mt-2 text-slate-300">{rawGraph ? `${rawGraph.nodes.length}개 노드 · ${rawGraph.edges.length}개 엣지` : ""}</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 w-80">
            ...
            <div className="space-y-3">
              <div>
                ...
                <select className="w-full border border-slate-200 rounded px-2 py-1.5 text-sm" value={newRel.from_id} onChange={(e) => setNewRel((p) => ({ ...p, from_id: e.target.value }))}>
                  <option value="">선택…</option>
                  {nodeIds.map((id) => <option key={id} value={id}>{rawGraph?.nodes.find((n) => n.id === id)?.label ?? id} ({id})</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">관계명</label>
                <input className="w-full border border-slate-200 rounded px-2 py-1.5 text-sm" placeholder="예: BELONGS_TO" value={newRel.relation} onChange={(e) => setNewRel((p) => ({ ...p, relation: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">To 엔티티</label>
                <select className="w-full border border-slate-200 rounded px-2 py-1.5 text-sm" value={newRel.to_id} onChange={(e) => setNewRel((p) => ({ ...p, to_id: e.target.value }))}>
                  <option value="">선택…</option>
                  {nodeIds.map((id) => <option key={id} value={id}>{rawGraph?.nodes.find((n) => n.id === id)?.label ?? id} ({id})</option>)}
                </select>
              </div>
            ...
            <div className="flex gap-2 mt-5">
              <button className="flex-1 bg-blue-600 text-white text-sm py-1.5 rounded hover:bg-blue-700 disabled:opacity-50" onClick={() => void handleAddRelationship()} disabled={!newRel.from_id || !newRel.relation.trim() || !newRel.to_id || addSubmitting}>
                {addSubmitting ? "추가 중…" : "추가"}
              </button>
              <button className="flex-1 bg-slate-100 text-slate-700 text-sm py-1.5 rounded hover:bg-slate-200" onClick={() => { setAddModal(false); setAddError(null); }}>취소</button>
            </div>
          </div>
        </div>
      )}
