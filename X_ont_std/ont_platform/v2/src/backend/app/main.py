"""ont_platform v2.0 — FastAPI entry point."""
...
app = FastAPI(title="ont_platform v2.0", version="2.0.0")
...
def _load_domain_json() -> dict:
    p = Path(__file__).resolve().parent / "config" / "domain.json"
    if not p.exists():
        return {"entity_types": [], "relation_types": []}
    return json.loads(p.read_text(encoding="utf-8"))

def _save_domain_json(data: dict) -> None:
    p = Path(__file__).resolve().parent / "config" / "domain.json"
...
def add_entity_type(
    ...
    if not name or not _re.match(r"^[A-Z][A-Z0-9_]*$", name):
        raise HTTPException(status_code=400, detail="유효한 엔티티 유형명이 필요합니다 (영대문자/숫자/_).")
    data = _load_domain_json()
...
def add_relation_type(
    ...
    new_rel = {
        "name": name,
        "from_type": (body.get("from_type") or "").strip().upper(),
