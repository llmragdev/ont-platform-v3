    ...
    def do_GET(self):
        ...
        <body>
            <div class="container">
                ...
                <h2>📝 작성된 코드</h2>
                <div class="code">
                    <code>import pandas as pd<br>import streamlit as st<br><br>st.set_page_config(page_title="공장 반복 고장 분석", layout="wide")<br>st.title("공장 반복 고장 분석")<br><br>data = pd.DataFrame([<br>    {"설비": "검사 카메라", "고장횟수": 3, "상태": "정비 필요"},<br>    {"설비": "배터리 탭 용접기", "고장횟수": 2, "상태": "관찰"},<br>])<br><br>metric_cols = st.columns(3)<br>metric_cols[0].metric("반복 고장 설비", len(data))<br>metric_cols[1].metric("총 고장 횟수", int(data["고장횟수"].sum()))<br>metric_cols[2].metric("정비 필요", int((data["상태"] == "정비 필요").sum()))<br><br>st.subheader("설비별 고장 횟수")<br>st.bar_chart(data.set_index("설비")["고장횟수"])<br><br>st.subheader("상세 목록")<br>st.dataframe(data, use_container_width=True)</code>
                </div>
