...
@app.post("/api/ontology/examples/order-submitted")
def seed_order_submitted_example(
    ctx: TenantContext = Depends(get_tenant_context),
    svc: OntologyService = Depends(get_ontology_service),
):
    doc_id = "order-example"
    schema = _load_domain_json(ctx)
    entity_types = schema.setdefault("entity_types", [])
    if not any(item.get("name") == "ORDER" for item in entity_types):
        entity_types.append({
            "name": "ORDER",
            "description": "Order request used by the v5 demo query examples.",
            "properties": ["status", "amount", "customer", "owner", "submitted_at"],
        })
    relation_types = schema.setdefault("relation_types", [])
    if not any(item.get("name") == "PLACED_BY" for item in relation_types):
        relation_types.append({"name": "PLACED_BY", "from_type": "ORDER", "to_type": "ORGANIZATION"})
    _save_domain_json(schema, ctx)
    ...
    examples = [
        {
            ...
            "provenance": {"source": "v5 example seed", "scenario": "submitted-order-query"},
        },
        {
            "id": "order-1002",
            "type": "ORDER",
            "name": "Order 1002 - Submitted",
            "properties": {
                "status": "Submitted",
                "amount": 7600,
                "customer": "Daehan Materials",
    ...
    for entity in examples:
        svc.upsert_entity(doc_id, entity, ctx)

    return {
        "doc_id": doc_id,
        "entity_count": len(examples),
        "query": "Submitted 상태인 Order를 찾아줘",
        "message": "Order example data is ready.",
    }
...
app.include_router(hybrid.router_v5)
...
from app.api import performance_api
...
def _mapping_path(ctx: TenantContext) -> Path:
    p = _get_ontology_path(ctx.company_id, ctx.project_id) / "ontology_mappings.json"
    p.parent.mkdir(parents=True, exist_ok=True)
    return p


def _load_mappings(ctx: TenantContext) -> list[dict]:
    p = _mapping_path(ctx)
    if not p.exists():
        return []
    return json.loads(p.read_text(encoding="utf-8"))
...
def _save_mappings(mappings: list[dict], ctx: TenantContext) -> None:
    _mapping_path(ctx).write_text(json.dumps(mappings, ensure_ascii=False, indent=2), encoding="utf-8")


def _mapping_similarity(external_label: str, candidate_label: str) -> float:
    external_tokens = {token for token in _re.split(r"[^a-zA-Z0-9가-힣]+", external_label.lower()) if token}
    candidate_tokens = {token for token in _re.split(r"[^a-zA-Z0-9가-힣]+", candidate_label.lower()) if token}
    if not external_tokens or not candidate_tokens:
        return 0.5
    overlap = len(external_tokens & candidate_tokens)
    union = len(external_tokens | candidate_tokens)
    return round(max(0.5, overlap / union), 2)


@app.get("/api/ontology/mapping-candidates")
def mapping_candidates(
    ...
    for doc in docs:
        graph = svc.get_graph(doc.get("doc_id", ""), ctx)
        for entity in graph.get("entities", [])[:20]:
            ...
            if not entity_id:
                continue
            similarity = _mapping_similarity(external_label or external_uri, label)
            candidates.append({
                "id": entity_id,
                "label": label,
                "type": str(entity.get("type") or entity.get("entity_type") or "Entity"),
                "similarity": similarity,
                "reason": "Label overlap with external resource" if similarity > 0.5 else "Available ontology entity",
            })
        ...
        candidates = [
            {
                ...
                "label": "Project Alpha",
                "type": "Project",
                "similarity": 0.91 if "project" in (external_label or external_uri).lower() else 0.72,
                "reason": "Demo candidate until ontology entities are loaded",
            },
            {
                "id": "entity:supplier-42",
                "label": "Daehan Materials",
                "type": "Organization",
                "similarity": 0.68,
                "reason": "Demo candidate from sample RDF graph",
            },
        ]

    candidates = sorted(candidates, key=lambda item: item["similarity"], reverse=True)[:5]
    return {"external_uri": external_uri, "external_label": external_label, "candidates": candidates}


@app.post("/api/ontology/mappings")
def save_mapping_rule(body: dict, ctx: TenantContext = Depends(get_tenant_context)):
    mappings = _load_mappings(ctx)
    mapping_id = body.get("id") or f"mapping-{int(time.time() * 1000)}"
    rule = {
        "id": mapping_id,
        "externalUri": body.get("externalUri"),
        "externalLabel": body.get("externalLabel"),
        "internalEntityId": body.get("internalEntityId"),
        "internalLabel": body.get("internalLabel"),
        "relationshipType": body.get("relationshipType", "skos:closeMatch"),
        "confidence": body.get("confidence", 0),
        "comment": body.get("comment", ""),
        "approvalStatus": body.get("approvalStatus", "pending"),
        "createdAt": datetime.utcnow().isoformat() + "Z",
    }
    mappings = [item for item in mappings if item.get("id") != mapping_id]
    mappings.append(rule)
    _save_mappings(mappings, ctx)
    return rule
...
# Phase 3.5: Write-back & DLQ Management API
from app.api.writeback_endpoints import router as writeback_router
app.include_router(writeback_router)

# Phase 4: SPARQL & RDF API
from app.api.sparql_endpoints import router as sparql_router
app.include_router(sparql_router)

# Phase 8: Customer Replies API (LLM Webhook + MCP Relay)
try:
    from app.api.extn.customer_replies import router as customer_replies_router
    app.include_router(customer_replies_router)
except ImportError:
    logging.warning("Customer Replies API not available")

# Phase 8: Customer Question Events API (Webhook Trigger + Batch Polling)
try:
    from app.api.extn.customer_questions import router as customer_questions_router
    app.include_router(customer_questions_router)
except ImportError:
    logging.warning("Customer Question Events API not available")

# Scenario 2: Factory Event APIs (Webhook Trigger + Batch Polling)
try:
    from app.api.extn.factory_events import router as factory_events_router
    app.include_router(factory_events_router)
except ImportError:
