...
class LineageService:
    ...
    def resolve_full_chain(self, entity_id: str) -> Optional[LineageInfo]:
        """전체 혈통 체인 해석"""
        lineage = self.get_lineage(entity_id)
        if lineage is None:
            # 캐시에 없으면 빈 혈통 정보 반환
            lineage = LineageInfo(source_type=DataSourceType.DERIVED)
        return lineage
    ...
    def analyze_impact(self, entity_id: str) -> Dict[str, Any]:
        ...
        affected_count = len(downstream.nodes) - 1  # 자신 제외

        return {
            "entity_id": entity_id,
            "affected_entities_count": affected_count,
            "affected_entities": list(downstream.nodes.keys()),
            "graph": downstream.to_dict()
        }
    ...
    def detect_circular_dependencies(self, entity_id: str) -> bool:
        ...
        return _has_cycle(entity_id)

    def compute_data_quality_score(self, entity_id: str) -> float:
        """데이터 품질 점수 계산 (0-1)"""
        lineage = self.get_lineage(entity_id)

        if lineage is None or lineage.source_type == DataSourceType.USER_INPUT:
            # 사용자 입력은 높은 품질
            return 0.95
        ...
        if transform_count > 0:
            # 각 변환마다 2% 감소
            quality = max(0.0, quality - (transform_count * 0.02))

        # 데이터 품질 체인이 있으면 최소값 사용
        if lineage.data_quality_chain:
            chain_min = min(lineage.data_quality_chain) / 100.0 if lineage.data_quality_chain else 0.9
            quality = min(quality, chain_min)
