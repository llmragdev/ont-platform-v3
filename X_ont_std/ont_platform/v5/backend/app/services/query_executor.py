...
class OntologyQueryEngine:
    ...
    def execute(self, step: QueryAction, ctx: TenantContext, question: str, doc_ids: list[str] | None = None) -> dict[str, Any]:
        if step.engine != EngineType.ONTOLOGY:
            raise ValueError(f"Unsupported engine: {step.engine}")
        ...
        if step.action == ActionType.CALCULATE:
            ...
            return {**filtered, "action": ActionType.CALCULATE.value, "result": filtered["count"],
                    "trace": filtered["trace"] + ["ontology.calculate: returned count"]}
        return {"engine": EngineType.ONTOLOGY.value, "action": step.action.value,
                "items": [], "count": 0, "trace": [f"ontology.{step.action.value.lower()}: unsupported action"]}

    def _filter(self, step: QueryAction, ctx: TenantContext, question: str, doc_ids: list[str] | None) -> dict[str, Any]:
        ...
        if property_key and property_value:
            items = self.ontology_svc.filter_by_property(ctx, entity_type, property_key, property_value, doc_ids)
            trace = ["ontology.filter: property filter",
        ...
        return {
            "engine": EngineType.ONTOLOGY.value, "action": ActionType.FILTER.value,
            "items": items, "count": len(items),
            "params": {"entity_type": entity_type, "property_key": property_key, "property_value": property_value},
