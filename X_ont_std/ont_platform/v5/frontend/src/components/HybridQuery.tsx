...
export function HybridQuery() {
  ...
  async function handleSeedExample() {
    ...
    try {
      ...
      const docs = await api.ontologyMgmt.listDocs();
      setDocs(Array.isArray(docs) ? docs : []);
      setSelectedDocIds([seeded.doc_id]);
      ...
      <section className="panel">
        ...
        <div className="panel-body space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            <button
              type="button"
              className="text-xs bg-emerald-100 text-emerald-700 rounded-full px-2.5 py-1 hover:bg-emerald-200 disabled:opacity-60"
              onClick={() => void handleSeedExample()}
              disabled={seeding || asking}
            >
