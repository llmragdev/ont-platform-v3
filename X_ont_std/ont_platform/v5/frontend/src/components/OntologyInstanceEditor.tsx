...
interface EditModal { mode: "create" | "edit"; entity?: OntologyEntity; }
...
export function OntologyInstanceEditor() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const [docList, s] = await Promise.all([api.ontologyMgmt.listDocs(), api.ontologyMgmt.getSchema()]);
  ...
  const loadEntities = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.ontologyMgmt.listEntities(selectedDoc, { entity_type: typeFilter || undefined, page, size: PAGE_SIZE });
  ...
  async function handleModalSubmit() {
    ...
    try {
      ...
      if (modal?.mode === "create") {
        await api.ontologyMgmt.createEntity(selectedDoc, { type: modalType, name: modalName, properties: props });
        showToast(true, "엔티티 추가됨");
      } else if (modal?.mode === "edit" && modal.entity) {
        await api.ontologyMgmt.updateEntity(selectedDoc, modal.entity.id, { name: modalName, properties: props });
  ...
  async function handleDelete(entityId: string) {
    if (!selectedDoc) return;
    try {
      await api.ontologyMgmt.deleteEntity(selectedDoc, entityId);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div>
          ...
          <select className="border border-slate-200 rounded px-2 py-1.5 text-sm" value={selectedDoc} onChange={(e) => { setSelectedDoc(e.target.value); setPage(1); }}>
            {docs.length === 0 && <option value="">문서 없음</option>}
            {docs.map((d) => <option key={d.doc_id} value={d.doc_id}>{d.doc_id} ({d.entity_count}개)</option>)}
          </select>
        </div>
        <div>
          <label className="text-xs text-slate-500 mr-1">유형 필터</label>
          <select className="border border-slate-200 rounded px-2 py-1.5 text-sm" value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setPage(1); }}>
            <option value="">전체</option>
            {allTypeNames.map((n) => <option key={n} value={n}>{n}</option>)}
          </select>
        </div>
        <div className="flex items-center gap-1 border border-dashed border-slate-300 rounded px-2 py-1">
          <input
            className="text-sm outline-none w-28 placeholder:text-slate-400"
            placeholder="새 문서 ID"
            value={newDocId}
          ...
          <button
            ...
            disabled={!newDocId.trim() || creatingDoc}
          >
            {creatingDoc ? "생성 중…" : "만들기"}
          </button>
        </div>
        <div className="ml-auto flex gap-2">
          <button className="bg-blue-600 text-white text-sm px-3 py-1.5 rounded hover:bg-blue-700 disabled:opacity-50"
            onClick={openCreate} disabled={!selectedDoc || !canEditOntology}
      ...
      {docs.length === 0 && (
        <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 py-10 text-center">
          <p className="text-sm text-slate-500 mb-1">온톨로지 문서 공간이 없습니다.</p>
          <p className="text-xs text-slate-400">위 입력란에 문서 ID를 입력하고 <strong>만들기</strong>를 눌러 새 공간을 만드세요.</p>
          <p className="text-xs text-slate-400 mt-1">예: <code className="bg-slate-200 rounded px-1">demo-doc-01</code> 또는 <code className="bg-slate-200 rounded px-1">purchase-orders</code></p>
        </div>
      )}
      ...
      <div className="rounded-xl border border-slate-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading && <tr><td colSpan={5} className="px-3 py-8 text-center text-slate-400 text-xs">로딩 중…</td></tr>}
            {!loading && entities.length === 0 && (
              <tr><td colSpan={5} className="px-3 py-8 text-center text-slate-400 text-xs">
                {selectedDoc ? "엔티티가 없습니다. 직접 추가하세요." : "문서를 선택하세요."}
              </td></tr>
            )}
            {entities.map((e) => (
              <tr key={e.id} className="border-b border-slate-100 hover:bg-slate-50">
                <td className="px-3 py-2 font-mono text-xs text-slate-500">{e.id}</td>
                <td className="px-3 py-2"><span className="bg-slate-100 text-slate-700 text-xs rounded px-1.5 py-0.5 font-medium">{e.type}</span></td>
                ...
                <td className="px-3 py-2 text-xs text-slate-500 max-w-xs truncate">
                  {Object.keys(e.properties).length > 0 ? Object.entries(e.properties).map(([k, v]) => `${k}: ${v}`).join(" · ") : <span className="text-slate-300 italic">없음</span>}
                </td>
                <td className="px-3 py-2 flex gap-1">
                  {canEditOntology && (
                    <button className="text-blue-500 hover:text-blue-700 text-xs px-1.5 py-0.5 rounded hover:bg-blue-50" onClick={() => openEdit(e)}>수정</button>
                  )}
                  {canEditOntology && (
                    <button className="text-rose-400 hover:text-rose-600 text-xs px-1.5 py-0.5 rounded hover:bg-rose-50" onClick={() => void handleDelete(e.id)}>삭제</button>
                  )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 text-sm">
          <button className="px-2 py-1 rounded border text-slate-600 disabled:opacity-40 hover:bg-slate-50" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}>이전</button>
          <span className="text-slate-500">{page} / {totalPages}</span>
          <button className="px-2 py-1 rounded border text-slate-600 disabled:opacity-40 hover:bg-slate-50" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}>다음</button>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 w-[480px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <div>
                ...
                {modal.mode === "create" ? (
                  <select className="w-full border border-slate-200 rounded px-2 py-1.5 text-sm" value={modalType} onChange={(e) => setModalType(e.target.value)}>
              ...
              <div>
                ...
                <input className="w-full border border-slate-200 rounded px-2 py-1.5 text-sm" value={modalName} onChange={(e) => setModalName(e.target.value)} />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">속성 (JSON)</label>
                <textarea className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs font-mono h-28 resize-none" value={modalProps} onChange={(e) => setModalProps(e.target.value)} />
              </div>
              {modalError && <p className="text-xs text-rose-600 bg-rose-50 rounded p-2">{modalError}</p>}
            </div>
            <div className="flex gap-2 mt-5">
              ...
              <button className="flex-1 bg-slate-100 text-slate-700 text-sm py-1.5 rounded hover:bg-slate-200" onClick={() => setModal(null)}>취소</button>
            </div>
          </div>
        </div>
      )}
